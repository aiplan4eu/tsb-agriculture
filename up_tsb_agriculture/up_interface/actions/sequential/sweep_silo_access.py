...
class ActionSweepSiloAccess(InstantaneousAction):
    """ Instantaneous action related to 'sweep silo access/unloading point'. """
    ...
    def __init__(self,
        ...
        raise NotImplementedError('This action is currently not supported')
        # @todo: planning with compactors is not supported for the current version of sequential planning

        params = {self.ParameterNames.COMPACTOR.value: upt.Compactor,
                  self.ParameterNames.SILO_ACCESS.value: upt.SiloAccess}

        InstantaneousAction.__init__(self, self.ActionNames.SWEEP_SILO_ACCESS.value, **params)
    ...
    def __add_conditions(self,
        ...
        comp = self.parameter(self.ParameterNames.COMPACTOR.value)
    ...
    def __add_effects(self,
        ...
        condition_cleared = LT( Minus( mass_in_sap, compactor_mass_per_sweep(comp) ), get_up_real(0.1) )
