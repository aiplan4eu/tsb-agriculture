...
class ActionDriveHarvToFieldAndInit(DurativeAction):
    """ Durative action related to 'drive harvester to a field (if needed) and initialize the harvesting process'. """
    ...
    def __init__(self,
        ...
        harv_at_from = transit_distance = no_loc_from_object = None
        if loc_from_type is upt.MachineInitLoc:
        ...
        delta_time_after_arrival = 0.0
        delta_time_after_arrival = max(delta_time_after_arrival,
                                       problem_settings.cost_windows.waiting_harvest_opening_time)
        delta_time_after_arrival = max(delta_time_after_arrival,
        ...
        delta_time_after_start = 0.0
        delta_time_after_start = max(delta_time_after_start,
        ...
        if only_init:
            delta_time_after_arrival = max(delta_time_after_start, delta_time_after_arrival)
            action_duration = delta_time_after_arrival
            timing_end_transit = StartTiming()
        else:
            # here we assume that the transit duration is >= delta_time_after_start
            action_duration = Plus(transit_duration, delta_time_after_arrival)
            timing_end_transit = get_timing_before_end_timing(action_duration, delay=delta_time_after_arrival)

        timing_enable_waiting_harvest = get_timing_before_end_timing(action_duration,
                                                                     delay=(delta_time_after_arrival -
                                                                            max(0.0,
                                                                                problem_settings.cost_windows.waiting_harvest_opening_time)))
        timing_enable_arriving = get_timing_before_end_timing(action_duration,
                                                              delay=(delta_time_after_arrival -
                                                                     max(0.0,
                                                                         problem_settings.control_windows.enable_arriving_tvs_in_field_opening_time)))
        ...
        self.__add_effects(fluents_manager=fluents_manager,
                           ...
                           timing_enable_waiting_harvest=timing_enable_waiting_harvest,
                           timing_enable_arriving=timing_enable_arriving,
    ...
    def __add_conditions(self,
        ...
        field_plan_id = fluents_manager.get_fluent(fn.field_plan_id)
        ...
        add_precondition_to_action(self,
                                   Or(
                                       ...
                                       And(
                                           Equals(field_pre_assigned_harvester(field), harv),
                                           GT(field_pre_assigned_turn(field), 0),
        ...
        # the field is not being harvested (i.e. has no plan id)
        add_precondition_to_action(self, Equals(field_plan_id(field), infield_planner.NO_PLAN_ID), StartTiming())
        ...
        if only_init:
            ...
            add_precondition_to_action(self, Equals(field_id(field), field_access_field_id(field_access)),
                                       StartTiming())
            ...
            if harv_enabled_to_drive_to_loc is not None:
                # the action must be planned while the planning window is open
                add_precondition_to_action(self, harv_enabled_to_drive_to_loc(harv), StartTiming())
    ...
    def __add_effects(self,
                      ...
                      no_loc_from_object,
                      timing_end_transit,
                      timing_enable_waiting_harvest,
                      timing_enable_arriving,
        ...
        def sim_effects_cb(timing: Timing,
            ...
            _loc_from = _field_access = _no_loc_from_object = _harv_transit_time = _transit_duration = None
            if not only_init:
            ...
            if harv_transit_time(harv) in effects_values.keys():
                _harv_transit_time = float(state.get_value(harv_transit_time(_harv)).constant_value())
                if only_init:
                    _transit_duration = 0
                else:
                    _transit_distance = float(
                        state.get_value(transit_distance(_loc_from, _field_access)).constant_value())
            ...
            if harv_at_from is not None and harv_at_from(harv) in effects_values.keys():
            ...
            for fl, val in effects_values.items():
                ...
                if fl is field_harvester(field):
                    ...
                    EffectsHandler.append_value_to_callback_return(ret_vals, effects_values, fl,
                                                                   _no_loc_from_object)  # harv_at_from(harv)
                    ...
                    EffectsHandler.append_value_to_callback_return(ret_vals, effects_values, fl,
                                                                   get_up_real(
                                                                       _harv_transit_time + _transit_duration))  # harv_transit_time(harv)
        ...
        effects_handler.add(timing=StartTiming(), fluent=field_plan_id(field), value=Int(0),
                            value_applies_in_sim_effect=True)  # @todo: temporary to know that the field is assigned (could be changed for a bool)
        effects_handler.add(timing=StartTiming(), fluent=field_harvester(field), value=harv,
                            value_applies_in_sim_effect=False)
        effects_handler.add(timing=StartTiming(), fluent=harv_overload_count(harv), value=Int(0),
                            value_applies_in_sim_effect=True)
        effects_handler.add(timing=StartTiming(), fluent=harv_overload_id(harv), value=Int(0),
                            value_applies_in_sim_effect=True)
        effects_handler.add(timing=StartTiming(), fluent=harv_field_turn(harv), value=Plus(harv_field_turn(harv), 1),
                            value_applies_in_sim_effect=False)

        effects_handler.add(timing=StartTiming(), fluent=harv_free(harv), value=False, value_applies_in_sim_effect=True)
        effects_handler.add(timing=timing_end_transit, fluent=harv_free(harv), value=True,
                            value_applies_in_sim_effect=True)
        ...
        if not only_init:
            ...
            effects_handler.add(timing=timing_end_transit, fluent=harv_at_field(harv), value=field,
                                value_applies_in_sim_effect=False)

            effects_handler.add(timing=StartTiming(), fluent=harv_transit_time(harv),
                                value=Plus(harv_transit_time(harv), transit_duration),
                                value_applies_in_sim_effect=False)

        if harv_waiting_to_harvest is not None:
            effects_handler.add(timing=timing_enable_waiting_harvest, fluent=harv_waiting_to_harvest(harv),
                                value=Bool(True), value_applies_in_sim_effect=True)

        if harv_enabled_to_drive_to_loc is not None:
            effects_handler.add(timing=StartTiming(), fluent=harv_enabled_to_drive_to_loc(harv), value=Bool(False),
                                value_applies_in_sim_effect=True)

        if tvs_all_enabled_to_drive_to_field is not None:
            # Enable an x seconds window for the 'drive_tv_to_field' action
            effects_handler.add(timing=StartTiming(), fluent=tvs_all_enabled_to_drive_to_field(), value=Bool(True),
                                value_applies_in_sim_effect=True)
            effects_handler.add(
                timing=StartTiming(delay=problem_settings.control_windows.enable_driving_tvs_to_field_opening_time),
                fluent=tvs_all_enabled_to_drive_to_field(), value=Bool(False),
                value_applies_in_sim_effect=True)

        if tvs_all_enabled_to_arrive_in_field is not None:
            # Enable an x seconds window for the 'drive_tv_to_field' action
            effects_handler.add(timing=timing_end_transit, fluent=tvs_all_enabled_to_arrive_in_field(field),
                                value=Bool(True), value_applies_in_sim_effect=True)
            effects_handler.add(timing=timing_enable_arriving, fluent=tvs_all_enabled_to_arrive_in_field(field),
        ...
        if harv_enabled_to_overload is not None:
            ...
            effects_handler.add(timing=timing_end_transit, fluent=harv_enabled_to_overload(harv),
                                value=harv_overload_id(harv),
                                value_applies_in_sim_effect=False)  # @note: the harv_overload_id was already initialized at StartTiming and maybe set some time between StartTiming and timing_end_transit
